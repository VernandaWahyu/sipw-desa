<?php
include(app_path().'\FPDF\fpdf.php');
include(app_path().'\FPDF\exfpdf.php');
include(app_path().'\FPDF\easyTable.php');

$pdf = new exFPDF('P', 'mm', array(210,297));
$border = 0;
// Path Gambar Logp
$path = public_path() . '/assets/img/logo/logo.png';
$pdf->SetTitle('SKTM - '.$id->name.' - '.strtotime(now()));
$pdf->AddPage();
$pdf->SetFont('Arial','B',12);
$pdf->image($path,15,12,18);
$pdf->Cell(0,2,'',$border,1);
$pdf->Cell(0,5,'PEMERINTAH KABUPATEN PONOROGO',$border,1,'C');
$pdf->Cell(0,5,'KECAMATAN MLARAK',$border,1,'C');
$pdf->SetFont('Arial','B',13);
$pdf->Cell(0,5,'DESA MLARAK',$border,1,'C');
$pdf->SetFont('Arial','B',9);
$pdf->Cell(0,4,'Jl Mlarak Pulung N0.39, - Purworejo, Kode Pos 63472',$border,1,'C');
$pdf->Cell(0,4,'Email : pemerintahdesamlarak@gmail.com, Ig @pemdes_mlarak',$border,1,'C');
$pdf->Cell(0,4,'Website: https://mlarak.ponorogokab.go.id/desa/mlarak',0,1,'C');
$pdf->Cell(0,1,'',0,1,'C');
$table=new easyTable($pdf, '{200}', 'width:70; border:0; font-size:8; line-height:1.2; paddingX:0');
$table->rowStyle('min-height:1;paddingY:0.02;marginY:1');
 $table->easyCell('', 'colspan:4; bgcolor:#000;');
 $table->printRow();
 $table->endTable();

$pdf->Cell(0,3,'',$border,1,'C');
$pdf->SetFont('Arial','BU',13);
$pdf->Cell(0,6,'SURAT KETERANGAN TIDAK MAMPU',$border,1,'C');
$pdf->SetFont('Arial','',11);
$pdf->Cell(0,5,'Nomor :        /        /      /   /        ',0,1,'C');
$pdf->Cell(0,10,'',0,1,'C');
$pdf->MultiCell(0,5, "Yang bertanda tangan dibawah ini Kepala Desa Mlarak, Kecamatan Mlarak, Kabupaten Ponorogo menerangkan dengan sebenarnya, bahwa:");
$pdf->Cell(0,5,'',0,5,'C');

$tabel = new easyTable($pdf, '{3,60,10,150}','border:0; paddingX:2');

$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Nama', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->name, 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('NIK', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->nik, 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Jenis Kelamin', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
if ($id->jk == 0) {
    $jk = "Laki-Laki";
}else {
    $jk = "Perempuan";
}
$tabel->easyCell($jk, 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Tempat, Tanggal Lahir', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->tempatlahir.', '.date("d-m-Y", strtotime($id->tanggallahir)), 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Agama', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->agama, 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Pekerjaan', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->pekerjaan, 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Status Pernikahan', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->statuspernikahan, 'valign:T;'); 
$tabel->printRow();
$tabel->easyCell(" ", 'valign:T;'); 
$tabel->easyCell('Alamat', 'valign:T');
$tabel->easyCell(":", 'valign:T;'); 
$tabel->easyCell($id->alamat    , 'valign:T;'); 
$tabel->printRow();
$tabel->endTable();
$pdf->SetFont('Arial','',11);
$pdf->MultiCell(0,5, "Nama tersebut diatas adalah benar warga Desa Mlarak, Kecamatan Mlarak, Kabupaten Ponorogo. Berdasarkan keterangan yang ada pada kami benar bahwa yang bersangkutan tergolong keluarga yang tidak mampu.");
$pdf->MultiCell(0,5, "Surat Keterangan ini dibuat untuk ". $id->keterangan);
$pdf->Cell(0,5,'',0,5,'C');
$pdf->MultiCell(0,5, "Demikian surat keterangan ini dibuat, atas perhatian dan kerjasamanya kami ucapkan terima kasih.");
$pdf->Cell(0,10,'',0,5,'C');
$tabel = new easyTable($pdf, '{100, 20,100}','border:0; paddingX:2;align:{CCC}');
$tabel->easyCell('MENGETAHUI');
$tabel->easyCell(""); 
$tabel->easyCell('Mlarak, '. date(" d  M Y") ); 
$tabel->printRow();
$tabel->easyCell('CAMAT MLARAK');
$tabel->easyCell(""); 
$tabel->easyCell('KEPALA DESA MLARAK'); 
$tabel->printRow();
$pdf->Cell(0,20,'',0,5,'C');
$tabel->easyCell('________________________');
$tabel->easyCell(""); 
$tabel->easyCell('________________________'); 
$tabel->printRow();
$tabel->endTable();
$pdf->Output('I','SKTM - '.$id->name.' - '.strtotime(now()).'.pdf');
exit;
?>